...
class MaskRCNN:
    def __init__(self):
        # Loading Mask RCNN
        self.net = cv2.dnn.readNetFromTensorflow("/home/avinaash/ros_cv/src/cv_tf/src/dnn/frozen_inference_graph_coco.pb",
                                                 "/home/avinaash/ros_cv/src/cv_tf/src/dnn/mask_rcnn_inception_v2_coco_2018_01_28.pbtxt")
        ...
        self.classes = []
        with open("/home/avinaash/ros_cv/src/cv_tf/src/dnn/classes.txt", "r") as file_object:
    ...
    def draw_object_info(self, bgr_frame, depth_frame):
        ...
        for box, class_id, obj_center in zip(self.obj_boxes, self.obj_classes, self.obj_centers):
            ...
            self.depth_mm = depth_frame[cy, cx]
            ...
            cv2.putText(bgr_frame, "{} cm".format(self.depth_mm / 10), (x + 5, y + 60), 0, 1.0, (255, 255, 255), 2)
            cv2.rectangle(bgr_frame, (x, y), (x2, y2), color, 1)

        return bgr_frame, self.depth_mm
